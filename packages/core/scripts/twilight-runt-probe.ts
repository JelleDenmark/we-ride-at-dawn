/*
 * ARCHIVED 2026-08-09 — does not run against the current tree.
 *
 * Rescued from a stale agent worktree under .claude/worktrees/ during a
 * cleanup pass; this was the only copy anywhere, including git history.
 * Committed so the derivation below stays recoverable, NOT because it is a
 * working tool: it imports ../src/boss-trial, which was removed when the
 * Boss Trial was retired, and it profiles Twilight-Runt, which is out of
 * SHOP_UNIT_POOL (Gutter-Gourmand is this season's reskin of its kit).
 *
 * Kept for the finding in the header below — that a pure-health half
 * converts to EXACTLY +0 Trial score, because the Trial scores raw damage
 * while a health grant is a linear cushion against an exponential boss.
 * That is a structural result about how a mode's SCORING inverts unit
 * value, not a numbers artifact, and it generalizes past this one unit.
 *
 * Delete freely if it stops being useful — git history is now the record.
 */
/**
