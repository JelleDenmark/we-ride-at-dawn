...
describe('Grave-Leech (issue #135: lifedrain sustain)', () => {
  ...
  it('drains 2 back after each clash it survives', () => {
    // Foe hits for 2, drain heals 2 — the Leech holds at full forever while
    // it chews through the wave.
    const { events, result } = simulate(
      lineup({ defId: 'grave-leech' }),
      gauntletOf([dummy(2, 20)])
    );
  ...
  it('the drain clamps at maxHealth — a 1-damage scratch heals only 1 back', () => {
  ...
  it('scales LINEARLY with tier (2/4/6), never the exponential curve', () => {
    // ★2 Leech (9/18): a 6-attack foe bites for 6, the drain returns 4.
    const { events } = simulate(
  ...
  it('holds a long grind far better than an unhealed body of the same size (the sustain identity)', () => {
    ...
    // rides several times deeper than the unhealed body.
    expect(control.result.wavesCleared).toBeLessThan(3);
    expect(drained.result.wavesCleared).toBeGreaterThanOrEqual(control.result.wavesCleared + 3);
  });
});
