...
- **Dire Rat — give it an identity via flat damage reduction (TO IMPLEMENT + SIM).** Dire Rat (cost 8) is an expensive stat-stick with mediocre value/scrap. Add a `damageReduction: X` primitive on `UnitDef`, applied in `sim.ts`'s `applyDamage` to **`'attack'` damage only** (poison bypasses — armor doesn't stop rot), clamped so a hit always deals **≥1** (avoid an immortal front tank — cf. the Bone-Priest self-revive lesson). Set Dire Rat to **X≈2** and **sim vs swarm vs brute waves** to tune. Effect: distinctively strong vs swarm (many small hits), weak vs brute/big-hitters → daily-theme counter-play, and the game's first **armor primitive** (reusable for the damage-type RPS idea). Also helps the cost↔value problem (justifies the 8 cost). _(Alternative considered & set aside: "block the first attack each wave" — that's anti-BURST, a different identity; go there instead only if we want Dire Rat to counter big-hitters rather than swarm.)_
