...
We now treat the **Keyword Family** — `poison | defence | offence | summon | buff | sustain` — as the game's one classification of what a Unit does, owned by `@wrad/core` (`data/keyword-family.ts`) alongside `UnitDef` and `Effect`. It is keyed off the Ability's effect kind, deliberately not `archetype` (set on Enemies only — 15 defs, zero player Rats) and not `tribe` (an untyped string on roughly half the roster that the UI never surfaced). Each family owns one glyph and one colour, lifted from sprites already in `packages/app/src/replay/art/`, so the UI and the art read as one palette instead of two drifting ones.

That colour ships as a **pair**, because a sprite hex chosen to look right on a rat is not automatically readable as 10px type. `FAMILY_COLOR` is the sprite-true value and paints graphical marks — the 3px tile edge — where the 3:1 non-text threshold applies. `FAMILY_TEXT_COLOR` is the same hue lifted by the smallest step that clears 4.5:1 on `--surface`, and every glyph or word takes that one. Three of the six needed lifting (offence sat at 2.93:1, defence 3.45:1, sustain 3.76:1 — all below `--ink-dim`'s own 4.33:1, i.e. below ADR-0006's floor for anything under 12px); the other three are byte-identical across both maps, and `keyword-family.test.ts` asserts they stay that way so the palette can never drift into two independent sets of six.
...
- **A new Relic declares a family too.** `RelicDef.family` is a required field, which is the enforcement: a relic cannot be declared without saying what it does. Relics reuse the Unit families rather than getting a second vocabulary — pinning Glass Shard to a Rat genuinely does make that Rat more offensive, and one encoding the player learns once beats two that nearly rhyme. Classify by **mechanism, not flavour**: Weeping Boil's name and art register are pure blight, but it deals flat damage and applies no poison stacks, so it is `offence`. A green tile promising rot that never arrives is worse than no colour at all.
